...
w2 = Mcz
b2 = -Mcz
Xt = np.linspace(-np.pi/2, np.pi/2)
T = np.sin(0.6*Xt) /2+np.sin(0.5*Xt) /3+np.cos(0.7*Xt) /2-np.cos(0.5*Xt)/3+0.2
n_data = len(T)
...
def forward(x, w1, b1, w2, b2):  # 正向传播
    y1 = sigmoid(x, w1, b1)
    y2 = tanh(y1, w2, b2)
    return y1, y2
